...
def detExcecution(test, fleetSize, routeMaxCost, nodes):
    ...
    init_sol, eff_list = genInitSol(test, fleetSize, routeMaxCost, nodes)
    ...
    for _ in range(10):
        ...
        OBD = merging(False, test, fleetSize, routeMaxCost, nodes, eff_list)
        OBD.time = time.time() - start_time

    return OBD

def simExcecution(test, fleetSize, routeMaxCost, nodes):
    # select the best alpha value and efficiency list for computing enchanced savings
    # and generate an initial solution using the selected alpha value and efficiency list
    init_sol, eff_list = genInitSol(test, fleetSize, routeMaxCost, nodes)
    # set initial sol as Our Best Det sol in a Det environment
    ...
    while elapsed < test.maxTime:
        # use the merging process of the PJs heuristic to generate a new det sol
        new_detSol = merging(True, test, fleetSize, routeMaxCost, nodes, eff_list)
...
def merging(useBR, test, fleetSize, routeMaxCost, nodes, eff_list):
    ...
    while len(effList) > 0: # list is not empty
        position = 0
        if useBR == True:
            position = getRandomPosition(test, test.firstParam, test.secondParam, len(effList))
        else:
